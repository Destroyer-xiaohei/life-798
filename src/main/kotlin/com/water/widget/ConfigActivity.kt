...
class ConfigActivity : ComponentActivity() {
    ...
    /** 各设备当前是否正在出水（deviceId -> running），用于把启动 / 停止合并成一个按钮。 */
    private val deviceRunningState = MutableStateFlow<Map<String, Boolean>>(emptyMap())
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            WaterTheme(mode = ThemeSettings.mode(this)) {
                ...
                val deviceRunning by deviceRunningState.collectAsStateWithLifecycle()
                DashboardScreen(
                    state = state,
                    ...
                    onStopDevice = { deviceId -> stopDevice(deviceId) },
                    deviceRunning = deviceRunning,
                    onRequestDeviceStatus = { deviceIds -> refreshDeviceStatus(deviceIds) },
                    onClaimServer = { startActivity(Intent(this, ServerClaimActivity::class.java)) }
                )
            }
        }
            ...
            override fun onResult(json: org.json.JSONObject?, err: String?) {
                runOnUiThread {
                    ...
                    try {
                        ...
                        viewModel.reloadAccounts()
                        updateWidgets()
                        refreshDeviceStatus(devices.map { it.first })
                    } finally {
    ...
    /**
     * 批量查询设备是否正在出水，结果写入 [deviceRunningState]。
     * 查询失败时保留上一次已知状态，不打扰用户；界面会显示「状态检测中」。
     */
    private fun refreshDeviceStatus(deviceIds: List<String>) {
        if (destroyed) return
        val account = AccountStore.getCurrent(this) ?: return
        val appToken = account.appToken
        if (appToken.isNullOrBlank()) return
        val targets = deviceIds.filter { it.isNotBlank() }.distinct()
        if (targets.isEmpty()) return
        targets.forEach { did ->
            WaterApi.statusWithToken(appToken, did, WaterApi.StatusCallback { running, _ ->
                if (running == null) return@StatusCallback
                runOnUiThread {
                    if (destroyed) return@runOnUiThread
                    val next = deviceRunningState.value.toMutableMap()
                    if (next[did] != running) {
                        next[did] = running
                        deviceRunningState.value = next
                    }
                }
            })
        }
    ...
    /**
     * 每次回到前台时静默把本机账号同步到自建积分托管服务端，
     * 保证服务端始终持有最新的积分 Token；未配置 / 无账号 / 网络失败都不打扰用户。
     */
    private fun syncClaimServerSilently() {
        if (!ClaimServerClient.isConfigured(this)) return
        val accounts = AccountStore.list(this)
            .filter { (it.hasToken() || it.hasAppToken()) && !it.uid.isNullOrBlank() }
        if (accounts.isEmpty()) return
        ClaimServerClient.pushAccounts(this, accounts) { _, _ -> }
    }

    /** 停止当前设备出水；失败时保留设备状态并提示原因。 */
    private fun stopDevice(deviceId: String) {
        ...
        WaterApi.stopWithToken(account.appToken, deviceId, WaterApi.Callback { status ->
            runOnUiThread {
                if (destroyed) return@runOnUiThread
                toast(status)
                updateWidgets()

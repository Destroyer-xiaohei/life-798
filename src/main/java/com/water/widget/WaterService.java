...
public class WaterService extends Service {
    ...
    /** 当前存活的服务实例，供「用户主动停止」时直接结束会话（同进程内）。 */
    private static volatile WaterService activeInstance;
    ...
    /** 当前是否有正在进行的接水监测会话（用于磁贴/界面判断启动还是停止）。 */
    public static boolean isMonitoring() {
        return ACTIVE_RESERVATION_ID.get() != 0L;
    }

    /**
     * 用户主动停止设备出水后调用：立即结束接水监测并撤掉进行中的通知。
     *
     * 只对正在监测的同一台设备生效；返回 true 表示确实有一次会话被结束。
     */
    public static boolean stopMonitoring(final String did, final String title,
                                         final String text) {
        final WaterService service = activeInstance;
        if (service == null) return false;
        service.onMain(() -> service.finishCancelled(did, title, text));
        return true;
    }
    ...
    /** 用户在 App / 小部件 / 磁贴里点了停止，结束当前会话并提示已停止。 */
    private void finishCancelled(String did, String title, String text) {
        Session current = session;
        if (current == null) return;
        if (did != null && !did.isEmpty() && !did.equals(current.did)) return;
        finishWithResult(current, title, text, false);
    }
